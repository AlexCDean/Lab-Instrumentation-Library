...
# Note: The LD400P has one phyiscal input channel.
...
class LoadInterfaceLD400P(LoadInterface):
    ...
    def get_level_A(self):
        stripped_val = self._query('A?').strip(' A/V\r\n')
        return self._parse_string(stripped_val, float)

    def get_level_B(self):
        stripped_val = self._query('B?').strip('B A/V\r\n')
        return self._parse_string(stripped_val, float)
    ...
    def get_current_load(self):
        stripped_val = self._query('I?').strip('A\r\n')
        return self._parse_string(stripped_val, float)

    def get_voltage_load(self):
        stripped_val = self._query('V?').strip('V\r\n')
        return self._parse_string(stripped_val, float)
